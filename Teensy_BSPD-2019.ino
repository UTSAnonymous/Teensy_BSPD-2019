...
//SENSOR VALUES
const int sensorMCLimit = 32766;//16384 for 50%, 32766 for 100%
//Raw read values
...
//Raw Signal Ranges
const int32_t throttle1Min = 2500;
const int32_t throttle1Max = 6700;
const int32_t throttle2Min = 1450;
...
void loop(void)
{
  ...
  if(CANbus.read(rxmsg))
  {
    ...
    else if(rxmsg.id == CANID_REMOTE_CONTROL)
    {
      ...
      maxAllowedSpeed = rxmsg.buf[1];
      if( maxAllowedSpeed > maxVehicleSpeed ) maxAllowedSpeed = maxVehicleSpeed;
...
void readSensor(void)
{
  ...
  if( remoteControlEnabled == false) {
    throttleVal = sensorThrottle1Value;
  }
  else if( remoteControlEnabled == true) {
    // limit remote control speed from 0 to maxAllowedSpeed
    int vehicleSpeed = constrain(speedVal, 0, maxAllowedSpeed);

    //map the vehicleSpeed from 0 to maxVehicleSpeed to the sensorMCLimit and put a constrain around it
    // to make it stable.
    throttleVal = constrain(map(vehicleSpeed, 0, maxVehicleSpeed, 0, sensorMCLimit), 0, sensorMCLimit);
  }
